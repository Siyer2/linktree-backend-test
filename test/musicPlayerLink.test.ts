...
describe("Music Player Link Tests", () => {
  ...
  it("Invalid music platform", async function () {
    const response = await request(app)
      .post(`/user/${USER_ID}/link`)
      .send({
        title: "Music",
        ...
        linkTypeSpecificData: [
          ...
          { platform: "INVALID", redirectLink: "apple.com/song" },
        ],
      })
      .expect(400);
  ...
  it("Missing music redirect", async function () {
    const response = await request(app)
      .post(`/user/${USER_ID}/link`)
      .send({
        title: "Music",
  ...
  it("Successfully create music link", async function () {
    const postBody = {
      title: "Music",
      subtitle: "Song Name - Artists Name",
      dateCreated: "2022-03-25T10:56:14.319Z",
    ...
    expect(newLink.subtitle).toBe(postBody.subtitle);
