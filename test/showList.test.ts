import { Server } from "http";
import request from "supertest";
import app from "../src/app";
import { Link, LinkType } from "../src/link/link.entity";
import { ResultReturn } from "../src/link/linkSubtypes/BaseLink";

const USER_ID = "7b6a7baf-7beb-4aad-af54-d880db8fc0f5";
const TITLE_LONGER_THAN_144 =
  "hellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohellohello";

describe("Base Link Tests", () => {
  let server: Server;

  beforeAll(() => {
    server = app.listen(process.env.PORT);
  });

  afterAll(() => {
    server.close();
  });

  it("Fail if no other link items", async function () {
    const response = await request(app)
      .post(`/user/${USER_ID}/link`)
      .send({
        redirectLink: "https://me.com",
        dateCreated: "2020-03-25T10:56:14.319Z",
        linkType: LinkType.ShowList,
        title: "48H",
      })
      .expect(400);

    const error: ResultReturn = response.body;
    expect(error.error).toBe("INVALID_INPUT");
    expect(error.errorMessage).toBe(
      "A non-empty 'linkTypeSpecificData' array is a required parameter when creating a ShowList link"
    );
  });

  it("Invalid show status", async function () {
    const response = await request(app)
      .post(`/user/${USER_ID}/link`)
      .send({
        title: "Music",
        subtitle: "Song Name - Artists Name",
        dateCreated: "2022-03-25T10:56:14.319Z",
        userId: "7b6a7baf-7beb-4aad-af54-d880db8fc0f5",
        linkType: LinkType.MusicPlayer,
        linkTypeSpecificData: [
          {
            showStatus: "soldOut",
            venue: "The Forum",
            redirectLink: "https://shows.com/forum",
            title: "Apr 01, 2019",
          },
          {
            showStatus: "INVALID",
            venue: "The Center",
            redirectLink: "https://shows.com/center",
            title: "Apr 02, 2019",
          },
        ],
      })
      .expect(400);

    const error: ResultReturn = response.body;
    expect(error.error).toBe("INVALID_INPUT");
  });

  it("Missing show redirect", async function () {
    const response = await request(app)
      .post(`/user/${USER_ID}/link`)
      .send({
        title: "Music",
        subtitle: "Song Name - Artists Name",
        dateCreated: "2022-03-25T10:56:14.319Z",
        userId: "7b6a7baf-7beb-4aad-af54-d880db8fc0f5",
        linkType: LinkType.MusicPlayer,
        linkTypeSpecificData: [
          {
            showStatus: "soldOut",
            venue: "The Forum",
            redirectLink: "https://shows.com/forum",
            title: "Apr 01, 2019",
          },
          {
            showStatus: "notOnSale",
            venue: "The Center",
            title: "Apr 02, 2019",
          },
        ],
      })
      .expect(400);

    const error: ResultReturn = response.body;
    expect(error.error).toBe("INVALID_INPUT");
  });

  it("Missing show title", async function () {
    const response = await request(app)
      .post(`/user/${USER_ID}/link`)
      .send({
        title: "Music",
        subtitle: "Song Name - Artists Name",
        dateCreated: "2022-03-25T10:56:14.319Z",
        userId: "7b6a7baf-7beb-4aad-af54-d880db8fc0f5",
        linkType: LinkType.MusicPlayer,
        linkTypeSpecificData: [
          {
            showStatus: "soldOut",
            venue: "The Forum",
            redirectLink: "https://shows.com/forum",
            title: "Apr 01, 2019",
          },
          {
            showStatus: "notOnSale",
            venue: "The Center",
          },
        ],
      })
      .expect(400);

    const error: ResultReturn = response.body;
    expect(error.error).toBe("INVALID_INPUT");
  });

  it("Show title longer than 144 characters", async function () {
    const response = await request(app)
      .post(`/user/${USER_ID}/link`)
      .send({
        title: "Music",
        subtitle: "Song Name - Artists Name",
        dateCreated: "2022-03-25T10:56:14.319Z",
        userId: "7b6a7baf-7beb-4aad-af54-d880db8fc0f5",
        linkType: LinkType.MusicPlayer,
        linkTypeSpecificData: [
          {
            showStatus: "soldOut",
            venue: "The Forum",
            redirectLink: "https://shows.com/forum",
            title: "Apr 01, 2019",
          },
          {
            showStatus: "notOnSale",
            venue: "The Center",
            redirectLink: "https://shows.com/center",
            title: TITLE_LONGER_THAN_144,
          },
        ],
      })
      .expect(400);

    const error: ResultReturn = response.body;
    expect(error.error).toBe("INVALID_INPUT");
  });

  it("Successfully create link", async function () {
    const postBody = {
      redirectLink: "https://shows.com",
      title: "Shows",
      linkType: "showList",
      linkTypeSpecificData: [
        {
          showStatus: "soldOut",
          venue: "The Forum",
          redirectLink: "https://shows.com/forum",
          title: "Apr 01, 2019",
        },
        {
          showStatus: "notOnSale",
          venue: "The Center",
          redirectLink: "https://shows.com/center",
          title: "Apr 02, 2019",
        },
      ],
    };

    const response = await request(app)
      .post(`/user/${USER_ID}/link`)
      .send(postBody)
      .expect(200);

    const newLink: Link = response.body;
    expect(newLink.title).toBe(postBody.title);
    expect(newLink.redirectLink).toBe(postBody.redirectLink);
    expect(newLink.linkType).toBe(postBody.linkType);
    expect(newLink.user.id).toBe(USER_ID);
    expect(typeof newLink.id).toBe("string");
    expect(typeof newLink.dateCreated).toBe("string");
    expect(newLink.linkTypeSpecificData).toStrictEqual(
      postBody.linkTypeSpecificData
    );
  });
});
